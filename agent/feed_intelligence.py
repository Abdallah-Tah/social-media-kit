...
def run_intelligent_feed(
    ...
    for cluster in clusters:
        ...
        # Track opportunity history for delta if a previous snapshot exists.
        history_delta = _load_history_delta(rep.url)
...
def _load_history_delta(url: str) -> dict[str, Any]:
    """Compare current story against the most recent intelligence snapshot.

    Returns previous score, current score, and delta. Used for the
    opportunity-history panel.
    """
    if not INTELLIGENCE_DIR.exists():
        return {"previous": None, "delta": None, "trend": "new"}
    snapshots = sorted(INTELLIGENCE_DIR.glob("*.json"), key=lambda p: p.stat().st_mtime, reverse=True)
    if not snapshots:
        return {"previous": None, "delta": None, "trend": "new"}
    latest = snapshots[0]
    try:
        ...
        return {"previous": None, "delta": None, "trend": "unknown"}
    for card in data.get("cards", []):
        cluster = card.get("cluster", {})
        urls = cluster.get("urls", [])
        if url in urls or cluster.get("representative", {}).get("url") == url:
            previous = card.get("opportunity", {}).get("opportunity_score")
            return {"previous": previous, "delta": None, "trend": "seen"}
    return {"previous": None, "delta": None, "trend": "new"}
